...
# 1. If discount > 10 mark "High Discount".
sales_data$DiscountLevel <- ifelse(sales_data$Discount > 10, "High Discount", "Low Discount")

# 2. Classify orders into High / Medium / Low value by using if else.
sales_data$OrderValue <- ifelse(sales_data$FinalAmount > 50000, "High",
...
# 4. Calculate cumulative revenue by using while loop
i <- 1; total <- 0
while(i <= nrow(sales_data)){ total <- total + sales_data$Revenue[i]; i <- i + 1 }
total

# 5. Skip fashion category.
for(i in 1:nrow(sales_data)) if(sales_data$Category[i] != "Fashion") print(sales_data[i, ])
